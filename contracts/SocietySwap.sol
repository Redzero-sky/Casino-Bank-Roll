...
contract SocietySwap is Ownable {
    ...
    uint256 public constant MIN_WITHDRAW_TAX_PERCENTAGE = 1;
    uint256 public constant MAX_WITHDRAW_TAX_PERCENTAGE = 2000;
    ...
    function deposit(uint256 amount) external {
        ...
        uint256 excludeFromFee = amount - fee;

        if (providers[msg.sender].deposited == 0) {
            providers[msg.sender].deposited = excludeFromFee;
    ...
    function getWithdrawFee(uint256 _fee) external onlyOwner {
        require(_fee >= MIN_WITHDRAW_TAX_PERCENTAGE && _fee <= MAX_WITHDRAW_TAX_PERCENTAGE, "Incorrect value");
        withdrawFee = _fee;
    }
